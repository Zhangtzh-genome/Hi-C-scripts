        ...
        tryCatch(
                {
                        ...
                        map_chr$gc <- log(genomic_features_chr$gc[ind1])   ############ 直接是log（Feature）
                        ...
                        if ( opt$negative_binomial ) {
                          library("MASS")
                          fit <- glm.nb(raw~len+gc+map, data = map_fit)
                        }
                        else{
                          fit <- glm(raw~len+gc+map, data = map_fit, family = "poisson") ########### offset 去掉 ##############
                        }
                        coeff <- round(fit$coeff,4)
                        res$nor <- round(map_chr$raw / exp(coeff[1] + coeff[2] * map_chr$len + coeff[3] * map_chr$gc + coeff[4]*map_chr$map), 4)
